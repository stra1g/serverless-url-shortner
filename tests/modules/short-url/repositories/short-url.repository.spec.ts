...
describe('ShortUrlRepositoryImpl', () => {
  ...
  describe('createShortUrl', () => {
    ...
    it('should throw an error if creation fails', async () => {
      (docClient.send as jest.Mock).mockRejectedValueOnce(new Error('DynamoDB error'));
      ...
      await expect(repository.createShortUrl(mockShortUrl)).rejects.toThrow('Could not create short URL');
      expect(docClient.send).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('getShortUrlByIdentifier', () => {
    it('should return a short URL if found', async () => {
      ...
      (docClient.send as jest.Mock).mockResolvedValueOnce({ Item: mockShortUrl });
