...
export const redirectShortUrlController =
  (shortUrlService: ShortUrlService, metricsService: MetricsService) =>
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (http_method.toLowerCase() !== req.method.toLowerCase()) {
        res.status(405).json({
          error: `This URL only accepts ${http_method.toUpperCase()} requests`,
        });
        return;
      }
